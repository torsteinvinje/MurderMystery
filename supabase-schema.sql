...
-- ============================================================================
-- 13) SKJULT AGENDA — hidden-identity social-deduction game mode (OPT-IN)
--
-- Se supabase/migrations/00009_saboteur_game.sql for full dokumentasjon av
-- sikkerhetsmodellen. Kort oppsummert:
--   - Fullt additivt: bygger på eksisterende games/players/host_token/
--     player_token, ingen ny identitetsmodell, ingen andre tabeller endres.
--   - Av som standard overalt: app_feature_flags('SABOTEUR_GAME_ENABLED')
--     sjekkes som FØRSTE setning i hver eneste funksjon under, før noen
--     token i det hele tatt slås opp.
--   - Samme mønster som resten av fila: RLS på, ZERO policies, alt via
--     SECURITY DEFINER-RPC-er med hånd-bygget JSON.
--   - _saboteur_game_for_host / _saboteur_participant_for_player krever i
--     tillegg at den løste raden tilhører ANROPERENS EGEN fest — dette er
--     sperren mot å bytte om på id-er for å nå andres data.
-- ============================================================================
